#### Question 1.
_Key point:_ Configure an App Service plan for a SaaS web service with on‑prem SQL Server, a singleton WebJob per customer, deployment slots, network isolation, and minimized costs.
...
#### Question 2.
_Key point:_ Configure Kubernetes CRDs for migrating an Azure Function (currently triggered by an Azure Storage queue) to run with KEDA.
...
#### Question 3.
_Key point:_ Automate deployment of a web app from GitHub using Azure CLI commands.
...
#### Question 4.
_Key point:_ Trigger photo processing from Blob storage events to produce a mobile-friendly image within one minute.
...
#### Question 5.
_Key point:_ Ensure scripts run and resources are available before a swap operation by updating web.config with applicationInitialization in a deployment slot scenario.
...
#### Question 6.
_Key point:_ Evaluate enabling auto swap for the Testing slot to run scripts before swap operation.
...
#### Question 7.
_Key point:_ Ensure pre-swap scripts run by disabling auto swap, updating the app (e.g. with a statuscheck method), then re-enabling auto swap for the Production slot.
...
#### Question 8.
_Key point:_ Initiate photo processing by converting the Azure Storage account to BlockBlobStorage.
...
#### Question 9.
_Key point:_ Validate a client certificate for a web app using TLS mutual authentication.
...
#### Question 10.
_Key point:_ Deploy a Linux container for a Docker/Go application using minimal resource groups via Azure CLI commands.
...
#### Question 11.
_Key point:_ Provision an App Service Web App for a Docker-hosted ASP.NET Core app (Fourth Coffee) and map a custom domain using CLI commands.
...
#### Question 12.
_Key point:_ Grant an Azure Functions app access to Azure Key Vault without code changes, ensuring warm starts and VNet connectivity by using managed identity.
...
#### Question 13.
_Key point:_ Ensure high availability and responsiveness for a high-traffic website while minimizing costs.
...
#### Question 14.
_Key point:_ Deploy an initial release of a Java web app from GitHub to a staging deployment slot for evaluation before production.
...
#### Question 15.
_Key point:_ Process the 'tip' property in Cosmos DB documents for a food delivery payment service to ensure it is present and numeric.
...
#### Question 16.
_Key point:_ Prevent timeout in an HTTP-triggered Azure Function processing blob data using the Durable Function async pattern.
...
#### Question 17.
_Key point:_ Evaluate whether passing the HTTP trigger payload to a Service Bus queue (processed by a queue trigger function) avoids timeout issues.
...
#### Question 18.
_Key point:_ Assess if configuring Always On on an App Service hosting plan prevents timeout in a long-running blob processing function.
...
#### Question 19.
_Key point:_ Trigger photo processing using an Azure Function with a blob upload trigger for minimal delay.
...
#### Question 20.
_Key point:_ Asynchronously process transaction logs (create, update, delete, copy) from Azure Blob Storage for auditing purposes.
...
#### Question 21.
_Key point:_ Create a Dockerfile for an ASP.NET Core application (ContosoApp) that runs setupScript.ps1 during build and executes ContosoApp.dll on container start.
...
#### Question 22.
_Key point:_ Configure an Azure Function App to process images as quickly as possible by using a Blob Storage trigger on an App Service plan.
...
#### Question 23.
_Key point:_ Configure an ARM template for a Java development environment ensuring that a VMSS is created only after storage accounts, load balancer, and virtual network are in place.
...
#### Question 24.
_Key point:_ Determine if the Azure Function App code (processing orders from an Azure Queue) logs the time of order processing.
...
#### Question 25.
_Key point:_ Determine if the function retries up to five times (including the first try) when processing orders fails.
...
#### Question 26.
_Key point:_ Determine if the function processes multiple orders concurrently when a batch is received from the queue.
...
#### Question 27.
_Key point:_ Verify that the function outputs the order to an Orders table in Azure Table Storage.
...
#### Question 28.
_Key point:_ Confirm that the provided code creates an infinite lease using the Azure Storage Client library for .NET.
...
#### Question 29.
_Key point:_ Verify if the code at line 06 always creates a new blob using the Azure Storage Client library for .NET.
...
#### Question 30.
_Key point:_ Confirm that the finally block in the .NET Storage Client code releases the blob lease.
...
#### Question 31.
_Key point:_ Determine the minimum SLA for data recovery when blobs are moved to the archive tier after 30 days via lifecycle management.
...
#### Question 32.
_Key point:_ Provision a SQL API Cosmos DB account for a ticket reservation system with required consistency and ordering by configuring failover and multiple regions.
...
#### Question 33.
_Key point:_ Deploy a Python website to an Azure Web App using a container with a fixed image version from a private ACR.
...
#### Question 34.
_Key point:_ Configure an autoscale rule for an App Service scaling down when the average Active Message Count from a Service Bus queue is less than or equal to a threshold.
...
#### Question 35.
_Key point:_ Update blob metadata using methods provided by the Azure Storage Client library for .NET.
...
#### Question 36.
_Key point:_ Implement a solution to receive POS device data from 2,000 stores into Azure Blob storage using a partition key based on device identifier.
...
#### Question 37.
_Key point:_ Implement a .NET object that receives messages (which do not persist after processing) when an Azure VM finishes processing data.
...
#### Question 38.
_Key point:_ Configure lifecycle management for a GPv1 Premium storage account to move data to archive storage after 1 year by upgrading to GPv2 and copying данных.
...
#### Question 39.
_Key point:_ Create a .NET object for configuring and executing requests to a globally-distributed NoSQL database using the latest Cosmos DB SDK.
...
#### Question 40.
_Key point:_ Copy all data from an existing Azure storage account to a new one with automation, minimal user input, and recoverability.
...
#### Question 41.
_Key point:_ Retrieve an access token for Azure Storage from a VM using managed identity via the Azure Instance Metadata Service.
...
#### Question 42.
_Key point:_ Configure a Cosmos DB policy to support ordering in a query for a new page in an application using the SQL API.
...
#### Question 43.
_Key point:_ Implement an Azure Event Hub for traffic monitoring along six highways with maximal throughput and minimal latency.
...
#### Question 44.
_Key point:_ Deploy a microservices solution to an AKS cluster requiring reverse proxy, TLS termination, and configurable traffic routing.
...
#### Question 45.
_Key point:_ Implement filtering for an order processing system using a Service Bus topic with optimal throughput and evaluation speed.
...
#### Question 46.
_Key point:_ Determine the correct sequence of actions between the CDN and the Point of Presence (POP) for distributing a company logo image.
...
#### Question 47.
_Key point:_ Select partition keys for a Cosmos DB solution with millions of documents lacking distinct partition values.
...
#### Question 48.
_Key point:_ Verify that the application code creates a SalesOrders database with two containers for an e‑commerce web app using Cosmos DB.
...
#### Question 49.
_Key point:_ Verify that Container1 in the SalesOrders solution will contain two items.
...
#### Question 50.
_Key point:_ Verify that Container2 in the SalesOrders solution will contain one item.
...
#### Question 51.
_Key point:_ Implement a change feed processor solution for replicating and processing changes in a Cosmos DB container.
...
#### Question 52.
_Key point:_ Register an application in Azure AD following the correct sequence in the App Registrations blade.
...
#### Question 53.
_Key point:_ Implement multifactor authentication for an internal website using Azure AD without additional cost.
...
#### Question 54.
_Key point:_ Restrict an Azure AD group (Cosmos DB Creators) from доступа к ключам, используя соответствующую роль RBAC для Cosmos DB с Cassandra API.
...
#### Question 55.
_Key point:_ Configure authorization for a web app using Azure AD where user group membership determines permission levels (admin, normal, reader) – first proposed solution.
...
#### Question 56.
_Key point:_ Configure authorization for a web app using Azure AD where group membership claims in the JWT determine permission levels – second proposed solution.
...
#### Question 57.
_Key point:_ Configure authorization using application roles defined in the Azure AD manifest and assignment to groups – third proposed solution.
...
#### Question 58.
_Key point:_ Protect Azure Key Vault and its objects from accidental deletion with a 90‑day retention period.
...
#### Question 59.
_Key point:_ Configure Azure API Management authentication policy for a managed web service with HSTS that requires a valid HTTP authorization header for each request.
...
#### Question 60.
_Key point:_ Configure an Azure AD application for an ASP.NET Core web app managing photographs with RBAC on Blob Storage containers.
...
#### Question 61.
_Key point:_ Update the ASP.NET Core app to integrate Azure App Configuration for feature flags and secure access using authentication and authorization middleware.
...
#### Question 62.
_Key point:_ Design an approach to load application secrets from Azure Key Vault without хранения секретов в приложении и минимальными изменениями в Azure AD.
...
#### Question 63.
_Key point:_ Secure medical records by encrypting scanned patient intake forms using an Azure Key Vault key and storing the encrypted data in Blob Storage.
...
#### Question 64.
_Key point:_ Store patient intake forms in an Azure Cosmos DB database with Storage Service Encryption enabled for compliance.
...
#### Question 65.
_Key point:_ Store patient intake forms as Azure Key Vault secrets to prevent компрометации содержимого при скачивании.
...
#### Question 66.
_Key point:_ Configure Azure Disk Encryption for a Linux VM using Azure CLI to secure весь диск с использованием индустриальных стандартов.
...
#### Question 67.
_Key point:_ Implement authentication for an Azure API (hosted in App Service) to access other Azure resources without callers sending credentials.
...
#### Question 68.
_Key point:_ In Azure Front Door, verify if the MIME type of an inbound XML file is supported for Brotli compression.
...
#### Question 69.
_Key point:_ In Azure Front Door, verify if purging all cache assets on edge nodes is required for Brotli compression to take эффект.
...
#### Question 70.
_Key point:_ In Azure Front Door, verify if the compression type (Brotli) is supported for inbound files.
...
#### Question 71.
_Key point:_ Arrange PowerShell commands to retrieve a storage account key and store it as a secret in Key Vault with правильным переключением контекста.
...
#### Question 72.
_Key point:_ Evaluate using an X.509 certificate to authenticate a VM with ARM for obtaining an access token.
...
#### Question 73.
_Key point:_ Evaluate using the Reader RBAC role to authenticate a VM with ARM for obtaining an access token.
...
#### Question 74.
_Key point:_ Determine the appropriate signal type for creating alert rules in Azure Log Analytics for performance counters с поддержкой dimensions и единым уведомлением при создании и разрешении тревоги.
...
#### Question 75.
_Key point:_ Implement Azure Search in a .NET Core MVC application to enable searching by regular expressions for holiday accommodation providers.
...
#### Question 76.
_Key point:_ Edit workflows for an existing Logic App.
...
#### Question 77.
_Key point:_ Apply governance policies in a stateful ASP.NET Core 2.1 web app (PolicyApp) reacting to Azure Event Grid authentication events with fast processing of sign-out events.
...
#### Question 78.
_Key point:_ Configure the Azure AD app manifest for an internal SPA to support login and personalization based on group membership.
...
#### Question 79.
_Key point:_ Develop code to access a secret stored in Azure Key Vault using the latest Azure SDK.
...
#### Question 80.
_Key point:_ Acquire a token for the Microsoft Graph API using certificate-based authentication in an Azure AD registered app.
...
#### Question 81.
_Key point:_ Ensure dependency tracking for third-party database calls in an ASP.NET Core Web API using Application Insights.
...
#### Question 82.
_Key point:_ Configure Azure CDN caching rules for a video-on-demand web app to cache every unique URL with a 1‑hour expiration.
...
#### Question 83.
_Key point:_ Improve performance of a D1-tier web app experiencing increased load by switching to a plan that supports autoscaling and configuring scale rules based on CPU load.
...
#### Question 84.
_Key point:_ Automatically move blobs to Archive tier after 180 days using lifecycle management and queue non-archived paths via a Logic App.
...
#### Question 85.
_Key point:_ Determine if the minimum throughput for a Cosmos DB container (with autoscaleMaxThroughput = 5000) is 400 R/Us.
...
#### Question 86.
_Key point:_ Verify if the query "SELECT * FROM c WHERE c.EmployeeId > '12345'" is an in‑partition query given the partition key '/EmployeeId'.
...
#### Question 87.
_Key point:_ Verify if the query "SELECT * FROM c WHERE c.UserID = '12345'" is a cross‑partition query given that the partition key is '/EmployeeId'.
...
#### Question 88.
_Key point:_ Determine if a mobile app using OAuth 2 implicit grant type requires any changes in registration (например, добавление секретов) in Azure AD.
...
#### Question 89.
_Key point:_ Identify which Azure Application Insights Usage Analysis features to use for выявления трендов в ASP.NET Core MVC приложении.
...
#### Question 90.
_Key point:_ Evaluate lifecycle management rules that move blobs (with prefixes container1/salesorders or container2/inventory) to cool storage after 60 days and to archive after 120 days.
...
#### Question 91.
_Key point:_ Evaluate if blobs are moved to cool storage if they have not been accessed for 30 days as per the applied policy.
...
#### Question 92.
_Key point:_ Determine if blobs tiered to cool will automatically be re-tiered to hot if к ним осуществляется повторный доступ.
...
#### Question 93.
_Key point:_ Evaluate if all block blobs older than 730 days will be deleted according to the lifecycle management policy.
...
#### Question 94.
_Key point:_ Determine the best storage option for user agreements in a social networking solution ensuring доступность независимо от отдельных сервисов.
...
#### Question 95.
_Key point:_ Create an Azure Monitor metrics alert for ContentUploadService when CPU usage exceeds a threshold using the correct CLI command.
...
#### Question 96.
_Key point:_ Investigate HTTP server log output to diagnose HTTP 502 errors in ContentUploadService.
...
#### Question 97.
_Key point:_ Implement bindings for the CheckUserContent Azure Function to receive input from a queue and output to Blob storage.
...
#### Question 98.
_Key point:_ Add markup in the application manifest to implement the ContentReview role for user authentication and auditing.
...
#### Question 99.
_Key point:_ Add YAML markup in the application manifest to enable ContentUploadService to access Azure Storage access keys securely.
...
#### Question 100.
_Key point:_ Choose the appropriate hosting model for deploying the CheckUserContent Azure Function to meet security and cost requirements.
...
#### Question 101.
_Key point:_ Trigger validation testing of a new version of ContentAnalysisService using previous 7 days of content.
...
#### Question 102.
_Key point:_ Configure ContentUploadService deployment to meet network security requirements (internal VNet access, SSL with valid root certificate).
...
#### Question 103.
_Key point:_ Deploy an application manifest (YAML) for MyApp on an AKS cluster using kubectl from an Azure AD–joined device.
...
#### Question 104.
_Key point:_ Evaluate deploying the YAML manifest for MyApp using the docker client (docker run -it microsoft/azure-cli:0.10.17).
...
#### Question 105.
_Key point:_ Configure the ARM template’s platformUpdateDomainCount for a set of VMs in an Availability Set to maximize доступность при отказах.
...
#### Question 106.
_Key point:_ Migrate an Azure VM (VM1) from one Hyper-V host to another using Azure Resource Manager capabilities.
...
#### Question 107.
_Key point:_ Deploy the YAML manifest for application MyApp on an AKS cluster using kubectl apply from an Azure AD–joined device.
...
#### Question 108.
_Key point:_ Evaluate deploying MyApp on AKS using the docker client command (docker run …) instead of kubectl.
...
#### Question 109.
_Key point:_ Set the platformUpdateDomainCount in the ARM template for VMs in an Availability Set to maximize доступность.
...
#### Question 110.
_Key point:_ Evaluate designing an Azure WebJob to run on the same instances as a web app using the Triggered WebJob type to restrict execution to a single instance.
...
#### Question 111.
_Key point:_ Evaluate designing an Azure WebJob using the Continuous WebJob type to restrict execution to a single instance.
...
#### Question 112.
_Key point:_ Migrate an on‑premises MongoDB deployment to an Azure Cosmos DB account (using the MongoDB API) including the Data Management Gateway tool.
...
#### Question 113.
_Key point:_ Process Azure Blob storage events asynchronously using Azure Event Grid with an Azure Function subscriber to process transaction logs in order for compliance.
...
#### Question 114.
_Key point:_ Process Azure Blob storage events asynchronously using the Azure Monitor HTTP Data Collector API for transaction log processing.
...
#### Question 115.
_Key point:_ Implement dynamic data masking for the email_address field in an Azure SQL Database using T-SQL ALTER TABLE statement.
...
#### Question 116.
_Key point:_ Implement dynamic data masking using the Set-AzSqlDatabaseDataMaskingPolicy PowerShell cmdlet.
...
#### Question 117.
_Key point:_ Implement dynamic data masking using the Set-AzSqlDatabaseDataMaskingRule PowerShell cmdlet for the Customers table and email_address column.
...
#### Question 118.
_Key point:_ Secure sign-ins to an e‑Commerce web app by ensuring Azure App Service authentication uses Azure AD and Azure Key Vault via Managed Service Identity (MSI).
...
#### Question 119.
_Key point:_ Configure a web app that uses Azure AD for authentication to support multifactor authentication (MFA).
...
#### Question 120.
_Key point:_ When creating an Azure Key Vault via PowerShell, ensure that deleted objects are retained for 90 days by using the appropriate parameters.
...
#### Question 121.
_Key point:_ Ensure that users connecting to Azure AD from unidentified IP addresses are automatically instructed to change their passwords by configuring Azure Key Vault.
...
#### Question 122.
_Key point:_ Ensure that users connecting to Azure AD from unidentified IP addresses are instructed to change their passwords using Azure AD Identity Protection.
...
#### Question 123.
_Key point:_ Ensure that users connecting to Azure AD from unidentified IP addresses are instructed to change their passwords using Azure AD Privileged Identity Management (PIM).
...
#### Question 124.
_Key point:_ Allow database developers to connect to an Azure SQL Database using on‑premises Active Directory accounts with minimal authentication prompts in SSMS.
...
#### Question 125.
_Key point:_ Configure the application to allow recovery of accidentally deleted Azure Key Vault or objects for 90 days.
...
#### Question 126.
_Key point:_ Evaluate a solution for a streaming video web app that uses Azure Redis Cache to ensure high availability and constant streaming quality.
...
#### Question 127.
_Key point:_ Evaluate a solution for a streaming video web app that uses an Azure Content Delivery Network (CDN) to store data nearest to the user.
...
#### Question 128.
_Key point:_ Evaluate a solution for a streaming video web app that uses a Storage Area Network (SAN) for data storage.
...
#### Question 129.
_Key point:_ Resolve increased page load times for a D1-tier web app by implementing autoscaling when CPU load is above 80% with minimized costs.
...
#### Question 130.
_Key point:_ Configure back‑end authentication for a public-facing API hosted in an Azure App Service using Basic gateway credentials in API Management.
...
#### Question 131.
_Key point:_ Configure back‑end authentication for a public-facing API using Client cert gateway credentials for the HTTP(s) endpoint.
...
#### Question 132.
_Key point:_ Configure back‑end authentication for a public-facing API using Basic gateway credentials for the HTTP(s) endpoint.
...
#### Question 133.
_Key point:_ Configure back‑end authentication for a public-facing API using Client cert gateway credentials for the Azure resource.
...
#### Question 134.
_Key point:_ Implement Azure Search in a .NET Core MVC application to list holiday accommodation venues within a specific price range and distance to an airport.
...
#### Question 136.
_Key point:_ Apply governance policies in PolicyApp (stateful ASP.NET Core 2.1 web app) reacting to Event Grid authentication events with быстрым выполнением sign-out событий.
...
#### Question 137.
_Key point:_ Configure the Azure AD app manifest for an internal website (SPA) to support login and personalization based on group membership.
...
#### Question 138.
_Key point:_ Develop code to access a secret in Azure Key Vault using the new Azure SDK.
...
#### Question 139.
_Key point:_ Acquire a token for Microsoft Graph API using certificate-based authentication in an Azure AD registered app.
...
#### Question 142.
_Key point:_ Implement autoscaling for a web app experiencing spikes in traffic by switching from a D1 to a Standard App Service tier and configuring scale rules based on CPU load.
...
#### Question 143.
_Key point:_ Automatically move blobs to the Archive tier after 180 days using lifecycle management, and send paths of non-archived items to a queue via a Logic App.
...
#### Question 144.
_Key point:_ Determine if the minimum throughput for a Cosmos DB container (with autoscaleMaxThroughput set to 5000) is 400 R/Us.
...
#### Question 146.
_Key point:_ Verify if the query "SELECT * FROM c WHERE c.UserID = '12345'" is a cross‑partition query given the partition key '/EmployeeId'.
...
#### Question 147.
_Key point:_ For a mobile app using OAuth 2 implicit grant type, determine if a redirect URI or additional secret is needed for registration in Azure AD.
...
#### Question 148.
_Key point:_ Identify the Application Insights Usage Analysis features for an ASP.NET Core MVC app tracking web pages and custom events to reveal trends.
...
#### Question 149.
_Key point:_ Evaluate lifecycle management rules that move blobs (with specific prefixes) to cool storage after 60 days and to archive storage after 120 days.
...
#### Question 150.
_Key point:_ Verify if blobs are moved to cool storage if not accessed for 30 days according to the lifecycle policy.
...
#### Question 151.
_Key point:_ Determine if blobs tiered to cool are automatically re‑tiered to hot if accessed again.
...
#### Question 152.
_Key point:_ Verify if all block blobs older than 730 days will be deleted according to the lifecycle management policy.
...
#### Question 153.
_Key point:_ Determine the optimal storage option for user agreements in a social networking solution ensuring высокую доступность и отказоустойчивость.
...
#### Question 154.
_Key point:_ Create an Azure Monitor metrics alert for ContentUploadService based on CPU usage using the correct CLI command.
...
#### Question 155.
_Key point:_ Investigate HTTP server log output for diagnosing HTTP 502 errors in ContentUploadService.
...
#### Question 157.
_Key point:_ Add markup in the application manifest to implement the ContentReview role for content monitoring and auditing.
...
#### Question 158.
_Key point:_ Add YAML markup in the application manifest to allow ContentUploadService access to Azure Storage access keys using secret volumes.
...
#### Question 160.
_Key point:_ Trigger validation testing of a new ContentAnalysisService version using data from the previous 7 days.
...
#### Question 161.
_Key point:_ Configure ContentUploadService deployment to meet network security policies by ensuring internal VNet access and SSL with a valid root certificate.
...
#### Question 38.
_Key point:_ Configure lifecycle management for a GPv1 Premium storage account to move data to archive storage after 1 year by upgrading to GPv2 and copying data.
...
#### Question 54.
_Key point:_ Restrict an Azure AD group (Cosmos DB Creators) from accessing keys using an appropriate RBAC role for Cosmos DB with Cassandra API.
...
#### Question 62.
_Key point:_ Design an approach to load application secrets from Azure Key Vault without storing secrets in the application and with minimal changes to Azure AD.
...
#### Question 65.
_Key point:_ Store patient intake forms as Azure Key Vault secrets to prevent compromise of contents upon download.
...
#### Question 66.
_Key point:_ Configure Azure Disk Encryption for a Linux VM using Azure CLI to secure the entire disk using industry-standard encryption.
...
#### Question 69.
_Key point:_ In Azure Front Door, verify if purging all cache assets on edge nodes is required for Brotli compression to take effect.
...
#### Question 71.
_Key point:_ Arrange PowerShell commands to retrieve a storage account key and store it as a secret in Key Vault with proper context switching.
...
#### Question 74.
_Key point:_ Determine the appropriate signal type for creating alert rules in Azure Log Analytics for performance counters with dimensions and single alert notifications upon creation and resolution.
...
#### Question 79.
_Key point:_ Develop code to access a secret stored in Azure Key Vault using the new Azure SDK.
...
#### Question 83.
_Key point:_ Improve performance of a D1-tier web app experiencing increased load by implementing autoscaling when CPU load is above 80% with minimized costs.
...
#### Question 89.
_Key point:_ Identify the Application Insights Usage Analysis features to use for revealing trends in an ASP.NET Core MVC app tracking webpages and custom events.
...
#### Question 92.
_Key point:_ Determine if blobs tiered to cool will automatically be re‑tiered to hot if accessed again.
...
#### Question 94.
_Key point:_ Determine the optimal storage option for user agreements in a social networking solution ensuring high availability and fault tolerance.
...
#### Question 101.
_Key point:_ Trigger validation testing of a new version of ContentAnalysisService using data from the previous 7 days.
...
#### Question 105.
_Key point:_ Configure the ARM template’s platformUpdateDomainCount for a set of VMs in an Availability Set to maximize availability during maintenance.
...
#### Question 109.
_Key point:_ Set the platformUpdateDomainCount in the ARM template for VMs in an Availability Set to maximize availability.
...
#### Question 136.
_Key point:_ Apply governance policies in PolicyApp (stateful ASP.NET Core 2.1 web app) reacting to Event Grid authentication events with fast execution of sign-out events.
...
#### Question 162.
_Key point:_ Configure diagnostic settings for an Azure Logic App to send logs to Log Analytics.
...
#### Question 163.
_Key point:_ Implement a CI/CD pipeline for an Azure App Service using GitHub Actions.
...
#### Question 164.
_Key point:_ Secure an Azure Function using App Service Authentication with Azure AD.
...
#### Question 165.
_Key point:_ Scale an Azure Cosmos DB container using autoscale throughput settings.
...
#### Question 166.
_Key point:_ Implement CORS for an Azure Function App to allow requests from specific domains.
...
#### Question 167.
_Key point:_ Configure a Managed Identity for an Azure Logic App to securely access an Azure SQL Database.
...
#### Question 168.
_Key point:_ Enable diagnostic logging for an Azure Web App using Application Insights.
...
#### Question 169.
_Key point:_ Implement data encryption at rest for an Azure SQL Database.
...
#### Question 170.
_Key point:_ Configure Azure Blob Storage lifecycle management to delete blobs older than a specified period.
...
#### Question 171.
_Key point:_ Implement a caching strategy for an ASP.NET Core web app using Azure Cache for Redis.
...
#### Question 172.
_Key point:_ Configure API Management policies to transform responses from a backend API.
...
#### Question 173.
_Key point:_ Monitor CPU usage of an Azure VM using Azure Monitor and set up alerts.
...
#### Question 174.
_Key point:_ Implement role-based access control (RBAC) for an Azure Storage account.
...
#### Question 175.
_Key point:_ Secure an Azure Kubernetes Service (AKS) cluster by enabling network policies.
...
#### Question 176.
_Key point:_ Configure a CI/CD pipeline for containerized applications using Azure Container Registry (ACR) and AKS.
...
#### Question 177.
_Key point:_ Implement distributed tracing for a microservices application using Azure Application Insights.
...
#### Question 178.
_Key point:_ Configure an Azure Function to process messages from an Azure Service Bus queue with session support.
...
#### Question 179.
_Key point:_ Secure access to an Azure Storage account by restricting network access using service endpoints.
...
#### Question 180.
_Key point:_ Implement a strategy to handle transient faults in Azure SQL Database using retry logic in a .NET application.
...
#### Question 181.
_Key point:_ Configure secure communication between an Azure Logic App and an Azure Function using managed identities.
...
#### Question 182.
_Key point:_ Monitor an Azure VM's performance using Log Analytics and set up custom queries for diagnostics.
...
#### Question 183.
_Key point:_ Implement a webhook to receive notifications from Azure Event Grid in a serverless function.
...
#### Question 184.
_Key point:_ Secure API calls in an Azure Function by validating JWT tokens issued by Azure AD.
...
#### Question 185.
_Key point:_ Scale out an Azure Function App to handle a sudden increase in queue messages.
...
#### Question 186.
_Key point:_ Enable logging for a containerized application running in Azure Container Instances.
...
#### Question 187.
_Key point:_ Deploy an Azure Logic App that integrates with Office 365 to send email notifications based on triggers.
...
#### Question 188.
_Key point:_ Configure backup for an Azure SQL Database using built-in automated backups.
...
#### Question 189.
_Key point:_ Implement secure storage of application secrets using Azure App Configuration with Key Vault references.
...
#### Question 190.
_Key point:_ Migrate on-premises applications to Azure using Azure Migrate to assess readiness and cost.
...
#### Question 191.
_Key point:_ Secure an Azure API Management instance with a custom domain and SSL certificate.
...
#### Question 192.
_Key point:_ Implement a serverless API to fetch currency exchange rates using Azure Functions and integrate with an external API.
...
#### Question 193.
_Key point:_ Implement distributed tracing for inter-service calls in a microservices architecture deployed in AKS.
...
#### Question 194.
_Key point:_ Configure an Azure Storage account to use Azure CDN for faster content delivery globally.
...
#### Question 195.
_Key point:_ Implement a solution for cross-region disaster recovery for an Azure Cosmos DB account.
...
#### Question 196.
_Key point:_ Secure an Azure Service Bus namespace by configuring Shared Access Policies with limited permissions.
...
#### Question 197.
_Key point:_ Implement a solution to monitor changes in an Azure Cosmos DB container using the change feed processor library.
...
#### Question 198.
_Key point:_ Enable secure communication between an Azure Web App and an Azure SQL Database using managed identity and firewall rules.
...
#### Question 199.
_Key point:_ Implement logging and diagnostics for an Azure Kubernetes Service (AKS) cluster using Azure Monitor for containers.
...
#### Question 200.
_Key point:_ Configure a CI/CD pipeline for an Azure Function App using Azure DevOps.
...
#### Question 201.
_Key point:_ Secure a multi-tenant application by implementing role-based access control (RBAC) using Azure AD groups.
...
#### Question 202.
_Key point:_ Implement a solution to trigger an Azure Logic App from an HTTP request with payload validation.
...
#### Question 203.
_Key point:_ Migrate an on-premises SQL Server database to Azure SQL Database with minimal downtime.
...
#### Question 204.
_Key point:_ Implement a serverless API gateway for microservices using Azure Functions Proxies.
...
#### Question 205.
_Key point:_ Enable real-time chat functionality in an application using Azure SignalR Service.
...
#### Question 206.
_Key point:_ Implement automated image recognition using Azure Cognitive Services in a web app.
...
#### Question 207.
_Key point:_ Monitor and alert on anomalies in an application using machine learning capabilities in Azure Monitor.
...
#### Question 208.
_Key point:_ Secure an API exposed via Azure API Management by enforcing a rate limit to prevent abuse.
...
#### Question 209.
_Key point:_ Implement caching in an Azure Function App to reduce calls to a backend database.
...
#### Question 210.
_Key point:_ Configure an Azure Storage account to use soft delete for blobs to enable data recovery.
...
#### Question 211.
_Key point:_ Implement a solution for secure file transfer between on-premises systems and Azure Blob Storage using Azure Data Box Gateway.
...
#### Question 212.
_Key point:_ Monitor performance and diagnostics for an Azure SQL Database using Query Performance Insight.
...
#### Question 213.
_Key point:_ Implement secure access to an Azure Key Vault using firewall rules and virtual network service endpoints.
...
#### Question 214.
_Key point:_ Secure an Azure Function App by restricting outbound traffic to specific virtual networks.
...
#### Question 215.
_Key point:_ Implement logging for an Azure Logic App using diagnostics settings to capture run history and failures.
...
#### Question 216.
_Key point:_ Implement an API to fetch currency exchange rates using Azure Functions and integrate with an external API.
...
#### Question 217.
_Key point:_ Secure a multi-tenant application using Azure AD for authentication and enforce group-based access control.
...
#### Question 218.
_Key point:_ Implement a background job to process high-volume email notifications using Azure Functions with a Timer trigger.
...
#### Question 219.
_Key point:_ Secure communications between a web app and an API by enforcing HTTPS in Azure App Service.
...
#### Question 220.
_Key point:_ Implement a serverless workflow using Azure Logic Apps to orchestrate multiple Azure Functions for data processing.
